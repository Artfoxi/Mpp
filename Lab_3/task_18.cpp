...
struct ListNode {
  ...
  ListNode(const std::string & str): data(str) {
      ...
      pthread_mutexattr_settype( & mutex_attr, PTHREAD_MUTEX_ERRORCHECK); // Для отладки
...
pthread_mutex_t list_mutex; // Мьютекс для заголовка списка (для операций добавления/удаления)
...
void split_string(const std::string & input) {
  ...
  while (pos < len) {
    ...
    pthread_mutex_lock( & list_mutex); // Защищаем добавление в список
    string_list.push_front(ListNode(chunk)); // Добавляем в начало списка
...
void print_list() {
  pthread_mutex_lock( & list_mutex); // Защищаем доступ к списку
  if (string_list.empty()) {
    ...
    for (auto & node: string_list) {
      pthread_mutex_lock( & node.mutex); // Защищаем доступ к данным узла
...
// Функция пузырьковой сортировки с мьютексами
void bubble_sort(std::list < ListNode > & lst) {
  ...
  do {
    ...
    while (next_it != lst.end()) {
      auto prev_it = it;
      if (it == lst.begin()) {
        // Если текущий элемент — первый, предыдущего нет
        prev_it = lst.end();
      } else {
        prev_it = std::prev(it);
      }
      ...
      if (prev_it != lst.end()) {
        pthread_mutex_lock( & prev_it -> mutex); // Предыдущий узел
      }
      pthread_mutex_lock( & it -> mutex); // Текущий узел
      pthread_mutex_lock( & next_it -> mutex); // Следующий узел
