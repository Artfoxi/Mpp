...
int main(int argc, char * argv[]) {
  ...
  std::vector < Connection > connections(MAX_CLIENTS);
    ...
    for (size_t i = 0; i < fds.size(); ++i) {
      ...
      if (fds[i].fd == listen_fd) {
        ...
        for (size_t j = 0; j < connections.size(); ++j) {
          if (connections[j].client_fd == -1) {
            connections[j].client_fd = client_fd;
            connections[j].is_client = true;
            fds.push_back({
              client_fd,
              POLLIN,
              0
        ...
        if (is_client) {
          ...
          if (header_end != std::string::npos) {
            ...
            if (cache.find(cache_key) != cache.end()) {
              conn.response = cache[cache_key].response;
          ...
          if (!conn.headers_sent) {
            ...
            if (header_end != std::string::npos) {
              ...
              write(conn.client_fd, conn.response.c_str(), conn.response.size());
              std::string cache_key = conn.host + conn.path;
              cache[cache_key].response = conn.response;
            }
          } else {
            write(conn.client_fd, buffer, n);
            std::string cache_key = conn.host + conn.path;
            cache[cache_key].response += buffer;
          }
        }
      }
    }
